...
/**
 * REST controller for managing {@link io.github.jhipster.sample.domain.Payment}.
 */
@RestController
@RequestMapping("/api")
@Transactional
    ...
    public ResponseEntity<Payment> partialUpdatePayment(
        ...
        Optional<Payment> result = paymentRepository
            ...
            .map(existingPayment -> {
                ...
                if (payment.getCreateDateTimeUtc() != null) {
                    existingPayment.setCreateDateTimeUtc(payment.getCreateDateTimeUtc());
                }
                if (payment.getStatus() != null) {
                    existingPayment.setStatus(payment.getStatus());
                }
                if (payment.getAmountToCollect() != null) {
                    existingPayment.setAmountToCollect(payment.getAmountToCollect());
                }
                if (payment.getSurchargeAmount() != null) {
                    existingPayment.setSurchargeAmount(payment.getSurchargeAmount());
                }
                if (payment.getConvertedTotalAmount() != null) {
                    existingPayment.setConvertedTotalAmount(payment.getConvertedTotalAmount());
                }
                if (payment.getConvertedCurrency() != null) {
                    existingPayment.setConvertedCurrency(payment.getConvertedCurrency());
                }
                if (payment.getConversionRate() != null) {
                    existingPayment.setConversionRate(payment.getConversionRate());
                }
