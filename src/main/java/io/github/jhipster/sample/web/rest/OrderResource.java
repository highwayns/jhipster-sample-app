...
/**
 * REST controller for managing {@link io.github.jhipster.sample.domain.Order}.
 */
@RestController
@RequestMapping("/api")
@Transactional
    ...
    public ResponseEntity<Order> partialUpdateOrder(@PathVariable(value = "id", required = false) final Long id, @RequestBody Order order)
        ...
        Optional<Order> result = orderRepository
            ...
            .map(existingOrder -> {
                ...
                if (order.getCreateDateTimeUtc() != null) {
                    existingOrder.setCreateDateTimeUtc(order.getCreateDateTimeUtc());
                }
                if (order.getCustomerReference() != null) {
                    existingOrder.setCustomerReference(order.getCustomerReference());
                }
