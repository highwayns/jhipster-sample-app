...
export class PaymentService {
  ...
  protected convertDateFromClient(payment: IPayment): IPayment {
    return Object.assign({}, payment, {
      createDateTimeUtc: payment.createDateTimeUtc?.isValid() ? payment.createDateTimeUtc.toJSON() : undefined,
    });
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((payment: IPayment) => {
        payment.createDateTimeUtc = payment.createDateTimeUtc ? dayjs(payment.createDateTimeUtc) : undefined;
      });
    }
    return res;
  }
